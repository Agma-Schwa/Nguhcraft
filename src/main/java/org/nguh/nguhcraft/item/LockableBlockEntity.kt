...
interface LockableBlockEntity {
     ...
     */
    fun CheckCanOpen(PE: PlayerEntity?, Key: ItemStack): Boolean {
        ...
        if (PE != null && ProtectionManager.Companion.BypassesRegionProtection(PE)) return true
        ...
        PE?.playSoundToPlayer(SoundEvents.BLOCK_CHEST_LOCKED, SoundCategory.BLOCKS, 1.0f, 1.0f)
        return false
    }
    ...
    fun FormatLockedMessage(Lock: String, BlockName: Text): MutableText = Text.translatable(
    ...
    fun IsLocked() = `Nguhcraft$GetLock`() != null
